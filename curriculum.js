...
<p><strong>Expected output:</strong> Clean dataset with no implausible values, proper factor levels, imputed ages, and a frequency table of industry.</p>`
      },
      {
        ...
        desc: "geoms, aesthetics, facets, themes",
        lesson: `<h3>Overview</h3>
<p>ggplot2 is the dominant visualization library in R and one of the most influential data visualization tools in any language. Before producing publication figures, you need fluency in the grammar of graphics — how layers, aesthetics, and scales compose into a complete plot.</p>
<h3>Key concepts</h3>
<ul>
<li><strong>Grammar of graphics</strong>: every plot is <code>data + aes() + geom_*()</code>. Aesthetics map variables to visual properties (x, y, color, size, shape).</li>
<li><strong>Common geoms</strong>: <code>geom_point</code>, <code>geom_line</code>, <code>geom_col</code>, <code>geom_histogram</code>, <code>geom_boxplot</code>, <code>geom_smooth</code>.</li>
<li><strong>facet_wrap / facet_grid</strong>: split a plot into panels by one or two variables — essential for comparing groups or regions.</li>
<li><strong>scales</strong>: <code>scale_x_log10()</code>, <code>scale_color_manual()</code>, <code>scale_fill_brewer()</code> — control axis transformations and color palettes.</li>
<li><strong>theme()</strong>: customize non-data elements — axis text, legend position, panel background, grid lines.</li>
</ul>
<h3>Worked example</h3>
<pre>library(tidyverse)
set.seed(42)
...
df <- tibble(
  ...
  female = factor(rbinom(n, 1, 0.5), labels=c("Male","Female")),
  region = sample(c("NE","MW","S","W"), n, replace=TRUE),
)
df$lwage <- with(df, 1.0 + 0.10*educ + 0.02*exper
                     - 0.18*(female=="Female") + rnorm(n,0,0.4))

# Scatter: education vs log wage, by gender
ggplot(df, aes(x=educ, y=lwage, color=female)) +
  geom_point(alpha=0.35, size=1.5) +
  geom_smooth(method="lm", se=TRUE) +
  scale_color_manual(values=c("Male"="#2e52c2","Female"="#c0392b")) +
  facet_wrap(~region) +
  labs(x="Years of education", y="Log wage",
       color=NULL, title="Education–wage gradient by region and gender") +
  theme_minimal(base_size=12) +
  theme(legend.position="bottom")</pre>
<div class="watchout"><strong>Watch out:</strong> ggplot uses non-standard evaluation — variable names inside <code>aes()</code> are unquoted column names, not strings. Wrapping them in quotes or using <code>$</code> notation will cause errors. Also, <code>geom_bar()</code> counts observations by default; use <code>geom_col()</code> when you already have computed values to plot.</div>`,
        exercise: `<h3>Exercise — ggplot2 visualization</h3>
<p><strong>Scenario:</strong> Produce a three-panel exploratory visualization of a labor economics dataset for a client presentation.</p>
<pre>library(tidyverse)
set.seed(15)

n <- 800
df <- tibble(
  wage    = rlnorm(n, 3.0, 0.45),
  educ    = sample(8:20, n, replace=TRUE),
  exper   = sample(0:42, n, replace=TRUE),
  female  = factor(rbinom(n,1,0.5), labels=c("Male","Female")),
  sector  = sample(c("Mfg","Services","Govt","Retail"), n, replace=TRUE),
  union   = factor(rbinom(n,1,0.14), labels=c("Non-union","Union")),
)</pre>
<ul>
<li><strong>1.</strong> Make a histogram of <code>log(wage)</code> faceted by <code>female</code>, with different fill colors for each gender. Add a vertical line at the overall mean. Use <code>theme_minimal()</code>.</li>
<li><strong>2.</strong> Make a boxplot of <code>log(wage)</code> by <code>sector</code>, colored by <code>union</code> status. Flip coordinates so sector labels are readable. Add a clean title and axis labels.</li>
<li><strong>3.</strong> Make a scatter of <code>exper</code> vs <code>log(wage)</code> with a <code>geom_smooth(method="lm")</code> line, faceted by sector (2×2 grid). Use <code>alpha=0.3</code> for points.</li>
<li><strong>4.</strong> Save plot 2 to <code>wage_by_sector.pdf</code> at 7×4 inches using <code>ggsave()</code>.</li>
</ul>
<p><strong>Expected output:</strong> Three publication-ready plots; saved PDF; clean labels and themes throughout.</p>`
      },
    ]},
    { tier: "Core Econometrics", modules: [
      {
...
<p><strong>Expected output:</strong> Clean dataset with no implausible wages, proper education categories, labeled values, and imputed ages.</p>`
      ...
      {
        id: "st_datatypes",
        title: "Data types & variable management",
        desc: "numeric, string, dates, label systems",
        lesson: `<h3>Overview</h3>
<p>Stata has a strict and expressive type system. Understanding variable storage types, label systems, and date handling prevents subtle errors common when working with administrative datasets, survey microdata, and panel files from official statistical agencies.</p>
<h3>Key concepts</h3>
<ul>
<li><strong>Storage types</strong>: <code>byte</code>, <code>int</code>, <code>long</code>, <code>float</code>, <code>double</code>, <code>str#</code> — use the smallest type that fits; always use <code>double</code> for money and prices.</li>
<li><strong>Variable labels</strong>: <code>label variable wage "Hourly wage (USD)"</code> — appear in output and graphs.</li>
<li><strong>Value labels</strong>: <code>label define</code> + <code>label values</code> — map numeric codes to strings (e.g. 1="Male", 2="Female").</li>
<li><strong>Dates</strong>: Stata stores dates as integers relative to 1 Jan 1960. <code>%td</code> = daily, <code>%tm</code> = monthly, <code>%tq</code> = quarterly formats.</li>
<li><strong>compress</strong>: automatically downcasts variables to the smallest type without data loss — run before saving large datasets.</li>
</ul>
<h3>Worked example</h3>
<pre>clear all; set seed 1; set obs 500

gen byte   female    = rbinomial(1, 0.5)
gen int    educ      = round(8 + runiform()*12)
gen double wage      = exp(rnormal(3.0, 0.4))
gen int    startdate = mdy(1, 1, 2010) + floor(runiform()*365*10)
format startdate %td

label variable female    "Female indicator"
label variable educ      "Years of education"
label variable wage      "Hourly wage (USD)"
label variable startdate "Employment start date"

label define sex 0 "Male" 1 "Female"
label values female sex

gen byte sector = ceil(runiform()*4)
label define sec 1 "Manufacturing" 2 "Services" 3 "Government" 4 "Retail"
label values sector sec

compress
describe
codebook female sector startdate</pre>
<div class="watchout"><strong>Watch out:</strong> <code>float</code> stores only ~7 significant digits — wages stored as <code>float</code> introduce rounding errors. Always use <code>double</code> for continuous monetary variables. Run <code>describe</code> after importing data to catch type problems early.</div>`,
        exercise: `<h3>Exercise — Variable types and labels</h3>
<p><strong>Scenario:</strong> You receive a raw panel dataset with unlabeled numeric codes. Apply a complete labeling system and fix storage types.</p>
<pre>clear all; set seed 42; set obs 600
gen id        = _n
gen year      = 2018 + floor(runiform()*5)
gen gender    = ceil(runiform()*2)
gen race      = ceil(runiform()*4)
gen educ_code = ceil(runiform()*5)
gen wage      = exp(rnormal(3.1, 0.45))
gen hire_date = mdy(1,1,2015) + floor(runiform()*365*8)
gen tenure    = (mdy(6,1,2023) - hire_date)/365</pre>
<ul>
<li><strong>1.</strong> Set appropriate storage types: <code>byte</code> for indicator/code variables, <code>double</code> for wage. Run <code>compress</code> and note memory savings from <code>describe</code>.</li>
<li><strong>2.</strong> Add variable labels for all 8 variables.</li>
<li><strong>3.</strong> Define and attach value labels for <code>gender</code> (1=Male, 2=Female), <code>race</code> (1=White, 2=Black, 3=Hispanic, 4=Other), and <code>educ_code</code> (1=LT HS through 5=Graduate). Format <code>hire_date</code> as <code>%td</code>.</li>
<li><strong>4.</strong> Run <code>tabulate gender race</code> — confirm labels display. Run <code>codebook hire_date tenure</code>.</li>
</ul>
<p><strong>Expected output:</strong> Labeled codebook output; cross-tab showing string labels not numeric codes; correct date format display.</p>`
      },
      {
        id: "st_macros",
        title: "Macros, loops & program flow",
        desc: "local, global, forvalues, foreach, if/else",
        lesson: `<h3>Overview</h3>
<p>Stata's macro and loop system separates manual click-through analysis from scalable, reproducible code. Consulting projects routinely require the same model across 20 subgroups, 10 outcomes, or 5 specifications — macros and loops make this maintainable.</p>
<h3>Key concepts</h3>
<ul>
<li><strong>Local macros</strong>: <code>local name "value"</code>, referenced as <code>\`name'</code> — scoped to the current do-file or program.</li>
<li><strong>Global macros</strong>: <code>global name "value"</code>, referenced as <code>$name</code> — persist across do-files; use for project-wide paths and variable lists.</li>
<li><strong>forvalues</strong>: loop over a numeric range — <code>forvalues i = 1/10 { ... }</code>.</li>
<li><strong>foreach</strong>: loop over a list — <code>foreach var of varlist wage educ exper { ... }</code>.</li>
<li><strong>if/else in do-files</strong>: conditional program logic — distinct from the <code>if</code> qualifier used in data commands.</li>
</ul>
<h3>Worked example</h3>
<pre>clear all; set seed 7; set obs 800
gen wage   = exp(rnormal(3.0, 0.4))
gen educ   = round(8 + runiform()*12)
gen exper  = round(runiform()*40)
gen female = rbinomial(1, 0.5)
...
local controls "educ exper female"
global output  "~/Desktop/output"

* Summarize each control
foreach var of local controls {
    quietly summarize \`var'
    display "\`var': mean=" %6.3f r(mean) " sd=" %6.3f r(sd)
}

* Regression by region
forvalues r = 1/4 {
    quietly regress wage \`controls' if region==\`r', robust
    display "Region \`r': educ coef=" %6.4f _b[educ]
}
...
if \`run_checks' {
    assert wage > 0
    display "Data checks passed."
}</pre>
<div class="watchout"><strong>Watch out:</strong> Local macros expire when the do-file finishes. Define them in the same do-file where they are used. Use globals only for project-wide paths and variable lists defined in your master do-file — and document them clearly so collaborators know what they contain.</div>`,
        exercise: `<h3>Exercise — Macros and loops</h3>
<p><strong>Scenario:</strong> Run a systematic robustness check estimating a wage regression across four industry groups and three specification levels.</p>
<pre>clear all; set seed 99; set obs 1200
gen wage     = exp(rnormal(3.1, 0.45))
gen educ     = round(8 + runiform()*12)
gen exper    = round(runiform()*42)
gen female   = rbinomial(1, 0.5)
gen union    = rbinomial(1, 0.15)
gen industry = ceil(runiform()*4)
label define ind 1 "Mfg" 2 "Services" 3 "Govt" 4 "Retail"
label values industry ind
gen lwage = log(wage)</pre>
<ul>
<li><strong>1.</strong> Define three local macros: <code>spec1 = "educ exper"</code>, <code>spec2 = "educ exper female"</code>, <code>spec3 = "educ exper female union"</code>.</li>
<li><strong>2.</strong> Use <code>forvalues</code> to loop over industries 1–4. Within each, run OLS of <code>lwage</code> on spec3 with robust SEs and display the industry label and education coefficient.</li>
<li><strong>3.</strong> Use <code>foreach</code> to loop over the three spec locals. For each, run the full-sample regression and display the education coefficient and R².</li>
<li><strong>4.</strong> Add a conditional: if the spec3 education coefficient is negative, display "Warning: unexpected sign". It should not fire.</li>
</ul>
<p><strong>Expected output:</strong> 4 industry-level education coefficients; 3 specification results with R²; no warning message.</p>`
      },
    ]},
    { tier: "Core Econometrics", modules: [
      {
